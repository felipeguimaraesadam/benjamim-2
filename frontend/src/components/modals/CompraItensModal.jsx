...
const CompraItensModal = ({ isOpen, onClose, compra }) => {
    ...
    const DetailItem = ({ label, value }) => (
        <div className="flex justify-between py-1.5 border-b border-gray-100 dark:border-gray-700">
            ...
            <dd className="text-sm text-gray-800 dark:text-gray-200 text-right">{value || 'N/A'}</dd>
        </div>
    );

    const DetailTextareaItem = ({ label, value }) => (
        <div className="py-1.5">
            <dt className="text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{label}:</dt>
    ...
    return (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto h-full w-full flex justify-center items-center z-50 px-4">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-3xl mx-auto my-8"> {/* Increased max-w and added my-8 for scroll margin */}
                <div className="flex justify-between items-center mb-5 border-b border-gray-200 dark:border-gray-700 pb-3">
                    <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-200">Detalhes da Compra</h2>
                    <button
                        onClick={onClose}
                ...
                <div className="space-y-4 mb-6">
                    <h3 className="text-lg font-medium text-gray-700 dark:text-gray-300 mb-3">Informações Gerais</h3>
                    ...
                    <h3 className="text-lg font-medium text-gray-700 dark:text-gray-300 mb-3 pt-3">Valores</h3>
                     <dl className="divide-y divide-gray-100 dark:divide-gray-700 bg-white dark:bg-gray-700 shadow-sm ring-1 ring-gray-900/5 dark:ring-gray-600/20 sm:rounded-xl p-4">
                    ...
                    {compra.observacoes && (
                        <>
                            <h3 className="text-lg font-medium text-gray-700 dark:text-gray-300 mb-2 pt-3">Observações</h3>
                ...
                <h3 className="text-lg font-medium text-gray-700 dark:text-gray-300 mb-3 pt-3 border-t border-gray-200 dark:border-gray-700">Itens da Compra</h3>
                {(!itens || itens.length === 0) ? (
                    <p className="text-center text-gray-500 dark:text-gray-400 py-4">Nenhum item encontrado para esta compra.</p>
                ) : (
                        ...
                        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                {itens.map((item, index) => (
                                    <tr key={item.id || index} className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                                        <td className="px-4 py-3 font-medium text-gray-900 dark:text-gray-200 whitespace-nowrap">
                                            {item.material_nome || 'N/A'}
                                        </td>
                                        <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{item.quantidade}</td>
                                        <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{formatCurrency(item.valor_unitario)}</td>
                                        <td className="px-4 py-3 text-gray-700 dark:text-gray-300">{item.categoria_uso || 'N/A'}</td>
                                        <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{formatCurrency(item.valor_total_item)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
