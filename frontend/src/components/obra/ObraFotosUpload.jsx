...
function ObraFotosUpload({ obraId, onUploadSuccess }) {
  ...
  const handleSubmit = async event => {
    ...
    try {
      ...
      setSuccessMessage(
        `Foto "${response.data.imagem.split('/').pop()}" enviada com sucesso!`
      );
      setImagem(null);
      setDescricao('');
      ...
      if (err.response && err.response.data) {
        if (
          err.response.data.imagem &&
          Array.isArray(err.response.data.imagem)
        ) {
  ...
  return (
    <div className="my-6 p-5 border border-gray-200 rounded-lg shadow-md bg-white">
      <h3 className="text-lg font-medium leading-6 text-gray-900 mb-4">
        Adicionar Nova Foto à Obra
      </h3>
      <form onSubmit={handleSubmit}>
        <div className="mb-4">
          <label
            htmlFor="fotoObraInput"
