...
import MaoDeObraDetailModal from '../modals/MaoDeObraDetailModal';
import ServicoDetailModal from '../modals/ServicoDetailModal';
import DespesaExtraDetailModal from '../modals/DespesaExtraDetailModal';
...
const FinancialDashboard = ({ obra }) => {
  ...
  const renderModal = () => {
    ...
    switch (activeModal) {
      case 'materiais':
        return <MaterialDetailModal {...modalProps} />;
      case 'mao_de_obra':
        return <MaoDeObraDetailModal {...modalProps} />;
      case 'servicos':
        return <ServicoDetailModal {...modalProps} />;
      case 'despesas_extras':
        return <DespesaExtraDetailModal {...modalProps} />;
      default:
        return null;
    }
  };
    ...
    <>
      ...
      */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div
            className="p-4 bg-teal-100 rounded-lg shadow dark:bg-teal-500/30 dark:hover:bg-teal-500/40 cursor-pointer transform hover:scale-105 transition-transform duration-200"
            onClick={() => setActiveModal('materiais')}
          >
          ...
          {/* Card Custo de Mão de Obra */}
          <div
            className="p-4 bg-indigo-100 rounded-lg shadow dark:bg-indigo-500/30 dark:hover:bg-indigo-500/40 cursor-pointer transform hover:scale-105 transition-transform duration-200"
            onClick={() => setActiveModal('mao_de_obra')}
          >
            ...
            <p className="text-xl font-bold text-indigo-900 dark:text-indigo-100">
              {formatCurrency(custos.mao_de_obra)}
            </p>
          </div>
          {/* Card Custo de Serviços */}
          <div
            className="p-4 bg-cyan-100 rounded-lg shadow dark:bg-cyan-500/30 dark:hover:bg-cyan-500/40 cursor-pointer transform hover:scale-105 transition-transform duration-200"
          ...
          >
            ...
            <p className="text-xl font-bold text-cyan-900 dark:text-cyan-100">
              {formatCurrency(custos.servicos)}
            </p>
          </div>
          {/* Card Custo de Despesas Extras */}
          <div
            className="p-4 bg-orange-100 rounded-lg shadow dark:bg-orange-500/30 dark:hover:bg-orange-500/40 cursor-pointer transform hover:scale-105 transition-transform duration-200"
            onClick={() => setActiveModal('despesas_extras')}
          >
