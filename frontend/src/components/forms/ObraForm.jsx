...
const ObraForm = ({ initialData, onSubmit, onCancel, isLoading }) => {
    ...
    if (initialData) {
      ...
      setFormData({
        ...
        data_inicio: initialData.data_inicio
          ? initialData.data_inicio.split('T')[0]
          : '',
        data_prevista_fim: initialData.data_prevista_fim
          ? initialData.data_prevista_fim.split('T')[0]
          : '',
        data_real_fim: initialData.data_real_fim
          ? initialData.data_real_fim.split('T')[0]
          : '',
  ...
  const handleChange = e => {
    console.log(
      `[DEBUG ObraForm handleChange] name: ${e.target.name}, value: ${e.target.value}`
    );
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formData.nome_obra.trim())
      newErrors.nome_obra = 'Nome da obra é obrigatório.';
    ...
    if (
      ...
      formData.data_prevista_fim &&
      formData.data_inicio > formData.data_prevista_fim
    ) {
      newErrors.data_prevista_fim =
        'Data prevista de fim não pode ser anterior à data de início.';
    }
    // data_real_fim should not be before data_inicio if both are set
    if (
      formData.data_inicio &&
      formData.data_real_fim &&
      formData.data_real_fim < formData.data_inicio
    ) {
      newErrors.data_real_fim =
        'Data real de fim não pode ser anterior à data de início.';
    }
    // Validation for orcamento_previsto
    if (
      formData.orcamento_previsto &&
      isNaN(parseFloat(formData.orcamento_previsto))
    ) {
      newErrors.orcamento_previsto = 'Orçamento previsto deve ser um número.';
    } else if (
      formData.orcamento_previsto &&
      parseFloat(formData.orcamento_previsto) < 0
    ) {
      newErrors.orcamento_previsto =
        'Orçamento previsto não pode ser negativo.';
    }
    ...
    if (formData.area_metragem && isNaN(parseFloat(formData.area_metragem))) {
      ...
      formData.area_metragem &&
      parseFloat(formData.area_metragem) < 0
    ) {
    ...
    if (validateForm()) {
      ...
      const dataToSubmit = {
        ...
        responsavel: formData.responsavel
          ? parseInt(formData.responsavel, 10)
          : null,
    ...
    <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
      ...
      <div>
        ...
        <textarea
          ...
          onChange={handleChange}
          rows="3"
          className={`bg-gray-50 border ${errors.endereco_completo ? 'border-red-500' : 'border-gray-300'} text-gray-900 sm:text-sm rounded-md focus:ring-primary-500 focus:border-primary-500 block w-full px-3 py-2`}
        ></textarea>
        {errors.endereco_completo && (
          <p className="mt-1 text-sm text-red-600 flex items-center">
            <WarningIcon /> {errors.endereco_completo}
          </p>
        )}
      </div>
      <div>
      ...
      <div>
        ...
        {errors.orcamento_previsto && (
          <p className="mt-1 text-sm text-red-600 flex items-center">
            <WarningIcon /> {errors.orcamento_previsto}
          </p>
        )}
      </div>
      ...
      <div>
        ...
        />
        {errors.area_metragem && (
          <p className="mt-1 text-sm text-red-600 flex items-center">
            <WarningIcon /> {errors.area_metragem}
          </p>
        )}
      </div>
      ...
      <div>
        ...
        {errors.responsavel && (
          <p className="mt-1 text-sm text-red-600 flex items-center">
            <WarningIcon /> {errors.responsavel}
          </p>
        )}
        {errors.funcionarios && (
          <p className="mt-1 text-sm text-red-600 flex items-center">
            <WarningIcon /> {errors.funcionarios}
          </p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          />
          {errors.data_prevista_fim && (
            <p className="mt-1 text-sm text-red-600 flex items-center">
              <WarningIcon /> {errors.data_prevista_fim}
            </p>
          )}
        </div>
        <div>
          ...
          />
          {errors.data_real_fim && (
            <p className="mt-1 text-sm text-red-600 flex items-center">
              <WarningIcon /> {errors.data_real_fim}
            </p>
          )}
        </div>
      </div>
