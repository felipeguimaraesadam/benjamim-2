...
const LocacaoForm = ({
  ...
  const [selectedFuncionarioObject, setSelectedFuncionarioObject] = useState(null);
  ...
  useEffect(() => {
    if (initialData) {
      ...
      const funcionarioId = initialData.funcionario_locado?.id || initialData.funcionario_locado || '';
      ...
      setFormData({
        ...
        data_pagamento: initialData.data_pagamento ? new Date(initialData.data_pagamento).toISOString().split('T')[0] : '',
        observacoes: initialData.observacoes || '',
      });

      if (obraId && obras?.length > 0) {
        setSelectedObraObject(obras.find(o => o.id.toString() === obraId.toString()) || null);
      }

      if (initialData.equipe) {
        setLocacaoType('equipe');
        if (equipeId && equipes?.length > 0) {
          setSelectedEquipeObject(equipes.find(e => e.id.toString() === equipeId.toString()) || null);
        }
  ...
  const handleFuncionarioSelect = (funcionario) => {
    ...
    setFormData(prev => ({ ...prev, funcionario_locado: funcionario ? funcionario.id : '' }));
  ...
  const handleChange = e => {
    ...
    if (name === 'tipo_pagamento') {
      ...
      if (!newDataInicio) {
        newFormData = { ...newFormData, data_locacao_fim: '', data_pagamento: '' };
      } else {
  ...
  const handleSubmit = async e => {
    ...
    if (locacaoType === 'funcionario' && selectedFuncionarioObject && formData.tipo_pagamento && !isNaN(valorPagamentoFloat)) {
      ...
      if (defaultFieldKey) {
        ...
        const currentDefaultValue = currentDefaultValueString != null && currentDefaultValueString !== '' ? parseFloat(currentDefaultValueString) : null;
  ...
  const proceedWithSubmission = async userChoice => {
    ...
    if (userChoice === 'update_default_and_save' && selectedFuncionarioObject && updateDefaultPromptData.paymentType) {
      ...
      if (fieldToUpdate) {
        try {
          ...
          const updatedEmployee = { ...selectedFuncionarioObject, [fieldToUpdate]: updateDefaultPromptData.formValue.toFixed(2) };
    ...
    if (userChoice === 'save_only_no_prompt' || userChoice === 'save_only' || funcionarioDefaultUpdated || (userChoice === 'update_default_and_save' && !funcionarioDefaultUpdated && window.confirm('Falha ao atualizar o valor padrão. Salvar mesmo assim?'))) {
      try {
        ...
        if (backendErrors?.conflict_details && backendErrors?.funcionario_locado) {
          setTransferDetails({
            ...
            conflictMessage: typeof backendErrors.funcionario_locado === 'string' ? backendErrors.funcionario_locado : JSON.stringify(backendErrors.funcionario_locado),
          });
          ...
          for (const key in backendErrors) {
            ...
            newFormErrors[key] = Array.isArray(backendErrors[key]) ? backendErrors[key].join('; ') : typeof backendErrors[key] === 'string' ? backendErrors[key] : JSON.stringify(backendErrors[key]);
          }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {locacaoType === 'servico_externo' && (
          <div>
            ...
            <input type="text" name="servico_externo" id="servico_externo" value={formData.servico_externo} onChange={handleChange} className={`mt-1 block w-full bg-gray-50 border ${formErrors.servico_externo ? 'border-red-500' : 'border-gray-300'} text-gray-900 sm:text-sm rounded-md focus:ring-primary-500 focus:border-primary-500 px-3 py-2`} maxLength="255" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input type="date" name="data_locacao_inicio" id="data_locacao_inicio" value={formData.data_locacao_inicio} onChange={handleChange} required className="mt-1 block w-full bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-md focus:ring-primary-500 focus:border-primary-500 px-3 py-2" />
          </div>
          <div>
            <label htmlFor="data_locacao_fim" className="block text-sm font-medium text-gray-900">Data Fim Locação (Opcional)</label>
            <input type="date" name="data_locacao_fim" id="data_locacao_fim" value={formData.data_locacao_fim} onChange={handleChange} className="mt-1 block w-full bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-md focus:ring-primary-500 focus:border-primary-500 px-3 py-2" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6 pt-6 border-t">
          <div>
            ...
            <select name="tipo_pagamento" id="tipo_pagamento" value={formData.tipo_pagamento} onChange={handleChange} className={`mt-1 block w-full bg-gray-50 border ${formErrors.tipo_pagamento ? 'border-red-500' : 'border-gray-300'} text-gray-900 sm:text-sm rounded-md focus:ring-primary-500 focus:border-primary-500 px-3 py-2`}>
          ...
          <div>
            ...
            <input type="number" name="valor_pagamento" id="valor_pagamento" value={formData.valor_pagamento} onChange={handleChange} className={`mt-1 block w-full bg-gray-50 border ${formErrors.valor_pagamento ? 'border-red-500' : 'border-gray-300'} text-gray-900 sm:text-sm rounded-md focus:ring-primary-500 focus:border-primary-500 px-3 py-2`} placeholder="Ex: 150.00" step="0.01" />
          ...
          <div>
            ...
            <input type="date" name="data_pagamento" id="data_pagamento" value={formData.data_pagamento} onChange={handleChange} className="mt-1 block w-full bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-md focus:ring-primary-500 focus:border-primary-500 px-3 py-2" />
        ...
        <div className="mt-6 pt-6 border-t">
          ...
          <textarea name="observacoes" id="observacoes" value={formData.observacoes} onChange={handleChange} rows="3" className="mt-1 block w-full bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-md shadow-sm focus:ring-primary-500 focus:border-primary-500 px-3 py-2" placeholder="Adicione observações relevantes sobre a locação..."></textarea>
        </div>

        <div className="mt-6 pt-6 border-t">
          <label htmlFor="anexos" className="block text-sm font-medium text-gray-900">Anexos (PDF, Imagens)</label>
          <input type="file" name="anexos" id="anexos" onChange={handleFileChange} multiple className="mt-1 block w-full text-sm text-gray-900 border border-gray-300 rounded-md cursor-pointer bg-gray-50 focus:outline-none" />
        </div>

        <div className="flex justify-end space-x-3 pt-4">
          <button type="button" onClick={onCancel} disabled={isLoading} className="py-2 px-4 text-sm font-medium text-gray-800 bg-gray-200 rounded-md hover:bg-gray-300 focus:ring-4 focus:outline-none focus:ring-gray-300 disabled:opacity-50">Cancelar</button>
          <button type="submit" disabled={isLoading} className="py-2 px-4 text-sm font-medium text-white bg-primary-600 rounded-md hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 disabled:bg-primary-300 flex items-center justify-center">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg">
            ...
            <p className="mb-6 text-sm text-gray-700">
              O valor da {updateDefaultPromptData.paymentType} (R$ {updateDefaultPromptData.formValue?.toFixed(2)}) é diferente do padrão atual do funcionário (R$ {updateDefaultPromptData.defaultValue?.toFixed(2)}).<br />
              Deseja atualizar o valor padrão do funcionário para R$ {updateDefaultPromptData.formValue?.toFixed(2)}?
            </p>
            <div className="flex justify-end space-x-3">
              <button onClick={() => { setShowUpdateDefaultConfirmModal(false); proceedWithSubmission('cancel_operation'); }} className="py-2 px-4 text-sm font-medium text-gray-800 bg-gray-200 rounded-md hover:bg-gray-300">Cancelar Operação</button>
              <button onClick={() => { setShowUpdateDefaultConfirmModal(false); proceedWithSubmission('save_only'); }} className="py-2 px-4 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-md">Não, salvar locação sem atualizar</button>
              <button onClick={() => { setShowUpdateDefaultConfirmModal(false); proceedWithSubmission('update_default_and_save'); }} className="py-2 px-4 text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 rounded-md">Sim, atualizar e salvar locação</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showTransferConfirm && transferDetails && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg">
            ...
            <p className="mb-2 text-sm text-gray-700">{transferDetails.conflictMessage || 'Este funcionário já possui uma locação que conflita com as datas informadas.'}</p>
            <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md text-sm">
              ...
              <p><strong>Período Conflitante:</strong>{new Date(transferDetails.conflictingLocacao.data_inicio + 'T00:00:00').toLocaleDateString()} - {transferDetails.conflictingLocacao.data_fim ? new Date(transferDetails.conflictingLocacao.data_fim + 'T00:00:00').toLocaleDateString() : 'Indefinido'}</p>
            </div>
            <p className="mb-6 text-sm text-gray-700">Deseja transferir o funcionário para esta nova locação? A locação anterior na obra '{transferDetails.conflictingLocacao.obra_nome}' será finalizada em {new Date(new Date(transferDetails.newLocacaoData.data_locacao_inicio).getTime() - 86400000).toLocaleDateString()} (um dia antes) e seu custo será removido da obra anterior.</p>
            {formErrors.general && <p className="text-red-600 text-sm mb-3">{formErrors.general}</p>}
            <div className="flex justify-end space-x-3">
              <button onClick={() => { setShowTransferConfirm(false); setTransferDetails(null); setFormErrors({}); }} disabled={isTransferring} className="py-2 px-4 text-sm font-medium text-gray-800 bg-gray-200 rounded-md hover:bg-gray-300 disabled:opacity-50">Cancelar</button>
              <button onClick={handleConfirmTransfer} disabled={isTransferring} className="py-2 px-4 text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 disabled:bg-primary-300">{isTransferring ? 'Transferindo...' : 'Sim, Transferir Funcionário'}</button>
            </div>
          </div>
        </div>
      )}
