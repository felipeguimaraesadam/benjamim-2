...
const MaterialForm = ({ initialData, onSubmit, onCancel, isLoading }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.nome.trim())
      newErrors.nome = 'Nome do material é obrigatório.';
    if (!formData.unidade_medida)
      newErrors.unidade_medida = 'Unidade de medida é obrigatória.';
    else if (
      !unidadeMedidaOptions.find(opt => opt.value === formData.unidade_medida)
    ) {
  ...
  const handleSubmit = e => {
    ...
    if (e && typeof e.preventDefault === 'function') {
      // Keep if e might sometimes be an event
      e.preventDefault();
    }
    if (validateForm()) {
      ...
      const dataToSubmit = {
        ...formData,
        quantidade_em_estoque: parseFloat(
          formData.quantidade_em_estoque
        ).toFixed(2), // Ensure it's a string with 2 decimal places
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      {' '}
      {/* Changed from form to div, removed onSubmit */}
      <div>
        <label
          htmlFor="nome"
      ...
      <div>
        ...
        {errors.unidade_medida && (
          <p className="mt-1 text-sm text-red-600 flex items-center">
            <WarningIcon /> {errors.unidade_medida}
          </p>
        )}
      </div>
      <div>
        <label
          htmlFor="quantidade_em_estoque"
          className="block mb-2 text-sm font-medium text-gray-900"
        >
        ...
        {errors.quantidade_em_estoque && (
          <p className="mt-1 text-sm text-red-600 flex items-center">
            <WarningIcon /> {errors.quantidade_em_estoque}
          </p>
        )}
      </div>
      <div>
        <label
          htmlFor="nivel_minimo_estoque"
          className="block mb-2 text-sm font-medium text-gray-900"
        >
          Nível Mínimo de Estoque para Alerta
        </label>
        <input
          type="number"
        ...
        <p className="mt-1 text-xs text-gray-500">
          Defina como 0 para não receber alertas para este material.
        </p>
        {errors.nivel_minimo_estoque && (
          <p className="mt-1 text-sm text-red-600 flex items-center">
            <WarningIcon /> {errors.nivel_minimo_estoque}
          </p>
        )}
      </div>
