...
const UsuarioForm = ({ initialData, onSubmit, onCancel, isLoading }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.nivel_acesso)
      newErrors.nivel_acesso = 'Nível de acesso é obrigatório.';
  ...
  const handleSubmit = e => {
    ...
    if (validateForm()) {
      ...
      if (senha) {
        // If a new password is provided (for create or update)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
      ...
      <div>
        ...
        {errors.nivel_acesso && (
          <p className="mt-1 text-sm text-red-600 flex items-center">
            <WarningIcon /> {errors.nivel_acesso}
          </p>
        )}
      </div>
