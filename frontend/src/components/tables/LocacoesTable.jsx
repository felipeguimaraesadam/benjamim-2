          ...
          {locacoes.map(locacao => (
            <tr key={locacao.id} className="bg-white border-b hover:bg-gray-50">
              <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                {locacao.obra_nome || getObraNome(locacao.obra)}
              </td>
              <td className="px-6 py-4">
                {locacao.equipe_nome
                  ? `Equipe: ${locacao.equipe_nome}`
                  : locacao.funcionario_locado_nome
                    ? `Funcionário: ${locacao.funcionario_locado_nome}`
                    : locacao.servico_externo
                      ? `Externo: ${locacao.servico_externo}`
                      : 'N/A'}
              </td>
              <td className="px-6 py-4">
              ...
              <td className="px-6 py-4">
                {formatTipoPagamento(locacao.tipo_pagamento)}
              </td>
              <td className="px-6 py-4">
                {formatCurrency(locacao.valor_pagamento)}
              </td>
              <td className="px-6 py-4">
                {formatDateToDMY(locacao.data_pagamento)}
              </td>
              <td className="px-6 py-4">
                <span
                  className={`px-2 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${getLocacaoStatusInfo(locacao).colorClass} ${getLocacaoStatusInfo(locacao).textColorClass}`}
                >
