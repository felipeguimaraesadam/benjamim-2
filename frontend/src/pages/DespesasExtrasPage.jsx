...
const DespesasExtrasPage = () => {
  ...
  const fetchDespesasExtras = useCallback(async (page = 1) => {
    setIsLoading(true);
    setError(null);
    try {
  ...
  const handleFormSubmit = async (formData) => {
    ...
    try {
      ...
      errorMessage = errorMessage || (isEditing ? 'Falha ao atualizar despesa.' : 'Falha ao criar despesa.');

      setError(errorMessage); // For display in modal
      showErrorToast(errorMessage); // General toast
      console.error("Form Submit Despesa Extra Error:", errorData || err.message);
    } finally {
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      ...
      {error && !isLoading && !showFormModal && !showDeleteConfirm && despesas.length === 0 && (
