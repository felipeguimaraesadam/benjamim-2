...
const ObraDetailPage = () => {
  ...
  const handleGenerateReport = async (isSimple = false) => {
    if (!obra || !obra.id) {
    ...
    try {
      const response = await api.gerarRelatorioPDFObra(obra.id, isSimple);
  ...
  return (
    ...
    <div className="container mx-auto px-4 py-6 bg-gray-100 dark:bg-gray-900 min-h-screen">
      ...
      {obra && obra.id && (
        <div className="my-4 py-4 text-center md:text-right flex justify-end gap-4">
          <button
          ...
          <button
            ...
            disabled={isGeneratingPdf || !obra}
            className="bg-teal-600 hover:bg-teal-700 text-white font-semibold py-2 px-6 rounded-md shadow-md focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-opacity-75 disabled:opacity-60 disabled:cursor-not-allowed transition ease-in-out duration-150 dark:focus:ring-teal-400"
          >
            {isGeneratingPdf ? (
              <>
                <svg className="animate-spin -ml-1 mr-2 h-5 w-5 text-white inline" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
