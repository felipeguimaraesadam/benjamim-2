...
const ObraDetailPage = () => {
  ...
  const actualTodasAsCompras = useMemo(() => {
    return (
      todasAsComprasBruto?.results ||
      (Array.isArray(todasAsComprasBruto) ? todasAsComprasBruto : [])
    );
  }, [todasAsComprasBruto]);
  ...
  useEffect(() => {
    console.log(
      '[DEBUG ObraDetailPage] State updated: `todasAsComprasBruto`',
      todasAsComprasBruto
    );
  }, [todasAsComprasBruto]);
    ...
    ? [
        {
          ...
          value: parseFloat(obra.orcamento_previsto) || 0,
        },
        {
          name: 'Custo Total Realizado',
          value: parseFloat(obra.custo_total_realizado) || 0,
        },
      ]
    : [];

  const composicaoGastosDataObra =
    obra && obra.custos_por_categoria
      ? [
          {
            name: 'Materiais (Compras)',
            value: parseFloat(obra.custos_por_categoria.materiais) || 0,
          },
          {
            name: 'Locações/Serviços',
            value: parseFloat(obra.custos_por_categoria.locacoes) || 0,
          },
          {
            name: 'Despesas Extras',
            value: parseFloat(obra.custos_por_categoria.despesas_extras) || 0,
          },
        ].filter(item => item.value > 0)
      : [];

  const gastosPorCategoriaMaterialDataObra =
    gastosPorCategoriaMaterial && gastosPorCategoriaMaterial.length > 0
      ? gastosPorCategoriaMaterial
          .map(item => ({
            name: item.name,
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      {operationStatus.message && (
        <div
          className={`p-3 my-4 rounded-md text-sm shadow ${operationStatus.type === 'success' ? 'bg-green-100 text-green-800 border border-green-300' : 'bg-red-100 text-red-700 border border-red-300'}`}
          role="alert"
        >
          {operationStatus.message}
        </div>
      ...
      {errorObra && (
        <div className="p-4 text-center">
          <p className="text-red-500">
            Erro ao carregar dados da obra: {errorObra}
          </p>
        </div>
      )}
      {!isLoadingObra && !errorObra && !obra && (
        <div className="p-4 text-center">
        ...
        <>
          ...
          <div className="my-8 p-4 bg-gray-50 shadow-md rounded-lg">
            <h2 className="text-xl font-bold mb-6 text-gray-700 text-center">
              Análise Financeira Detalhada da Obra
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {/* Chart 1: Orçamento vs. Gasto Total */}
              <div className="p-4 bg-white shadow-lg rounded-lg">
                <h3 className="text-lg font-semibold mb-3 text-center text-gray-600">
                  Orçamento vs. Gasto Total
                </h3>
                {orcamentoVsGastoDataObra.length > 0 &&
                (orcamentoVsGastoDataObra[0].value > 0 ||
                  orcamentoVsGastoDataObra[1].value > 0) ? (
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      <Pie
                        data={orcamentoVsGastoDataObra}
                        dataKey="value"
                        nameKey="name"
                        cx="50%"
                        cy="50%"
                      ...
                      >
                        {orcamentoVsGastoDataObra.map((entry, index) => (
                          <Cell
                            key={`cell-ovg-${index}`}
                            fill={
                              COLORS_PIE_OBRA[index % COLORS_PIE_OBRA.length]
                            }
                          />
                        ))}
                      </Pie>
              ...
              <div className="p-4 bg-white shadow-lg rounded-lg">
                ...
                {composicaoGastosDataObra.length > 0 ? (
                      ...
                      <Bar dataKey="value" fill="#82ca9d">
                        {composicaoGastosDataObra.map((entry, index) => (
                          <Cell
                            key={`cell-cg-${index}`}
                            fill={
                              COLORS_CATEGORIES_OBRA[
                                index % COLORS_CATEGORIES_OBRA.length
                              ]
                            }
                          />
                        ))}
                  ...
                  <p className="text-center text-gray-500 py-10">
                    Não há composição de gastos para exibir.
                  </p>
                )}
              </div>
                ...
                {gastosPorCategoriaMaterialDataObra.length > 0 ? (
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      ...
                      >
                        {gastosPorCategoriaMaterialDataObra.map(
                          (entry, index) => (
                            <Cell
                              key={`cell-gcm-${index}`}
                              fill={
          ...
          <div className="mb-8 py-6">
            {' '}
            {/* Removed tab navigation structure, kept py-6 for padding */}
