  ...
  const fetchObras = useCallback(
    async (page = 1) => {
      ...
      try {
        ...
        const errorMsg =
          err.message || 'Falha ao buscar obras. Tente novamente.';
        setError(errorMsg);
  ...
  const handleFormSubmit = async formData => {
    ...
    try {
      ...
      const errorMsg =
        ...
        err.response?.data?.nome_obra?.[0] ||
        err.message ||
        (currentObra ? 'Falha ao atualizar obra.' : 'Falha ao criar obra.');
      setError(errorMsg); // Show error in the modal
      showErrorToast(errorMsg);
      console.error(
        'Form Submit Obra Error:',
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-md">
            ...
            <p className="mb-6 text-gray-700 dark:text-gray-300">
              Tem certeza que deseja excluir esta obra? Esta ação não pode ser
              desfeita.
            </p>
