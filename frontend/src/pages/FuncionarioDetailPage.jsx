      ...
      try {
        ...
        setError(
          err.response?.data?.error ||
            err.message ||
            'Erro ao buscar detalhes do funcionário.'
        );
      } finally {
        setIsLoading(false);
      }
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="bg-white shadow rounded-lg p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <p>
            <strong>Nome Completo:</strong> {funcionarioDetails.nome_completo}
          </p>
          <p>
            <strong>Cargo:</strong> {funcionarioDetails.cargo}
          </p>
          <p>
            <strong>Data de Contratação:</strong>{' '}
            {formatDateToDMY(funcionarioDetails.data_contratacao)}
          </p>
          <p>
            <strong>Valor Diária Padrão:</strong>{' '}
            {formatCurrency(funcionarioDetails.valor_diaria_padrao)}
          </p>
          <p>
            <strong>Valor Metro Padrão:</strong>{' '}
            {formatCurrency(funcionarioDetails.valor_metro_padrao)}
          </p>
          <p>
            <strong>Valor Empreitada Padrão:</strong>{' '}
            {formatCurrency(funcionarioDetails.valor_empreitada_padrao)}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white shadow rounded-lg p-6">
        ...
        {funcionarioDetails.ocorrencias_registradas &&
        funcionarioDetails.ocorrencias_registradas.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="min-w-full bg-white">
              <thead className="bg-gray-100">
                <tr>
              ...
              <tbody className="text-gray-700">
                {funcionarioDetails.ocorrencias_registradas.map(ocorrencia => (
                  ...
                  >
                    <td className="py-3 px-4">
                      {formatDateToDMY(ocorrencia.data)}
                    </td>
                    <td className="py-3 px-4">{ocorrencia.tipo}</td>
                    <td className="py-3 px-4">
                      {ocorrencia.observacao || 'N/A'}
                    </td>
                  </tr>
                ))}
              </tbody>
