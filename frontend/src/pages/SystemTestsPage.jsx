...
const SystemTestsPage = () => {
  ...
  const populateTestData = async () => {
    ...
    try {
      const response = await apiClient.post('/test-data/populate/');
      ...
      toast.error(`Erro ao adicionar dados de teste: ${error.response?.data?.error || error.message}`);
    } finally {
  ...
  const clearTestData = async () => {
    if (!window.confirm('Tem certeza que deseja remover todos os dados de teste? Esta ação não pode ser desfeita.')) {
      return;
    }
    
    setIsClearingData(true);
    try {
      const response = await apiClient.delete('/test-data/clear/');
      if (response.data.status === 'success') {
      ...
      toast.error(`Erro ao remover dados de teste: ${error.response?.data?.error || error.message}`);
    } finally {
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4 mb-6">
        ...
        <p className="text-sm text-yellow-700 dark:text-yellow-300 mb-4">
          Use estas opções para adicionar ou remover dados de teste no sistema. Os dados incluem obras, funcionários, equipes, materiais, compras e locações com diferentes cenários.
        </p>
        <div className="flex flex-wrap gap-3">
          <button
            onClick={populateTestData}
            disabled={isPopulatingData || isClearingData}
            className="bg-green-600 hover:bg-green-700 dark:bg-green-500 dark:hover:bg-green-600 text-white font-medium py-2 px-4 rounded-md focus:outline-none focus:ring-4 focus:ring-green-300 dark:focus:ring-green-400 disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
          >
            {isPopulatingData ? (
              <>
                <SpinnerIcon />
              ...
              <>
                ...
                <span>Adicionar Dados de Teste</span>
              </>
            )}
          </button>
          ...
          >
            {isClearingData ? (
              ...
              <>
                ...
                <span>Remover Dados de Teste</span>
              </>
            )}
          </button>
        </div>
