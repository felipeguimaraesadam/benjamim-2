    ...
    try {
      ...
      if (isEditing) {
        ...
        if (Array.isArray(createdRentals) && createdRentals.length > 1) {
          showSuccessToast(`${createdRentals.length} locações criadas com sucesso (uma para cada dia)!`);
        } else {
  ...
  return (
    <div className="container mx-auto px-4 py-6 bg-white dark:bg-gray-900 min-h-screen">
      ...
      <div className="mb-8 flex flex-col">
        <h2 className="text-2xl font-semibold text-gray-700 dark:text-gray-200 mb-4 flex-shrink-0">Planejamento Semanal de Locações</h2>
      ...
      <div className="mb-8 p-4 border border-gray-200 dark:border-gray-700 rounded-lg shadow bg-white dark:bg-gray-800">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-semibold text-gray-700 dark:text-gray-200">Custo Diário de Locações (Últimos 30 dias)</h2>
          <div className="flex items-center">
            <label htmlFor="obraChartFilter" className="mr-2 text-sm font-medium text-gray-700 dark:text-gray-300">Filtrar por Obra:</label>
            <select
              id="obraChartFilter"
        ...
        {isLoadingChart && <p className="text-center text-gray-500 dark:text-gray-400">Carregando gráfico...</p>}
        {chartError && (
          <div className="bg-red-100 dark:bg-red-900 border border-red-400 dark:border-red-500 text-red-700 dark:text-red-200 px-4 py-3 rounded relative" role="alert">
        ...
        {!isLoadingChart && !chartError && chartData.length === 0 && (
          <p className="text-center text-gray-500 dark:text-gray-400">Nenhum dado de locação encontrado para o período ou filtro selecionado.</p>
        )}
        {!isLoadingChart && !chartError && chartData.length > 0 && (
      ...
      <div className="flex justify-between items-center mb-6 mt-12">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Listagem Detalhada de Locações</h1>
         <div>
            <button
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-md">
            ...
            <p className="mb-6 text-gray-600 dark:text-gray-400">Tem certeza que deseja excluir esta locação? Esta ação não pode ser desfeita.</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 transition-opacity duration-300 ease-in-out">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            {step === 1 && (
              <div>
                <div className="mb-4">
                  <label htmlFor="locWeekSelector" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Selecionar Semana (Opcional):</label>
                  <select
                    id="locWeekSelector"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div>
                    <label htmlFor="locReportStartDate" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Data de Início <span className="text-red-500 dark:text-red-400">*</span></label>
                    <input
                      type="date"
                  ...
                  <div>
                    <label htmlFor="locReportEndDate" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Data de Fim <span className="text-red-500 dark:text-red-400">*</span></label>
                    <input
                      type="date"
                      id="locReportEndDate"
                      value={reportEndDate}
                ...
                {preCheckError && <p className="text-red-500 dark:text-red-400 text-sm mb-3">{preCheckError}</p>}
                <button
              ...
              <div className="my-4">
                ...
                {(preCheckAlertDays.length > 0 || preCheckMedicoesPendentes.length > 0) ? (
                  <>
                    ...
                    <p className="mb-3 text-gray-700 dark:text-gray-300">Deseja gerar o relatório mesmo assim?</p>
                    <div className="flex justify-end space-x-3">
                       <button onClick={() => setStep(1)} className="bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 text-gray-700 dark:text-gray-200 font-medium py-2 px-4 rounded-md">Voltar/Corrigir Datas</button>
                       <button onClick={handleContinueDespiteAlert} disabled={isGeneratingReport} className="bg-primary-600 hover:bg-primary-700 dark:bg-primary-500 dark:hover:bg-primary-600 text-white font-medium py-2 px-4 rounded-md disabled:opacity-50">
                  ...
                  <div className="p-4 bg-green-100 dark:bg-green-900 border-l-4 border-green-500 dark:border-green-400 text-green-700 dark:text-green-200">
                    ...
                    <p>Nenhuma pendência (dias sem locações ou medições zeradas) encontrada no período selecionado.</p>
                     <div className="flex justify-end space-x-3 mt-3">
                       <button onClick={() => setStep(1)} className="bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 text-gray-700 dark:text-gray-200 font-medium py-2 px-4 rounded-md">Voltar</button>
                       <button onClick={handleGenerateReport} disabled={isGeneratingReport} className="bg-primary-600 hover:bg-primary-700 dark:bg-primary-500 dark:hover:bg-primary-600 text-white font-medium py-2 px-4 rounded-md disabled:opacity-50">
                        {isGeneratingReport && <SpinnerIcon className="w-5 h-5 mr-2" />}
                ...
                {reportError && !isGeneratingReport && <p className="text-red-500 dark:text-red-400 text-sm mt-3">{reportError}</p>}
              </div>
            )}
            ...
            {step === 3 && reportData && (
              <div className="mt-6">
                ...
                {reportError && <p className="text-red-500 dark:text-red-400 text-sm mb-3">{reportError}</p>}
                {reportData.length === 0 && <p className="text-gray-600 dark:text-gray-400">Nenhuma locação encontrada para o período e critérios selecionados.</p>}

                {reportData.map(obraData => (
                  <div key={obraData.obra_id || obraData.obra_nome} className="mb-8 p-4 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-lg shadow">
                            ...
                            <table className="min-w-full text-xs text-left text-gray-600 dark:text-gray-400">
                              ...
                              <tbody>
                                {diaData.locacoes_no_dia.map(loc => (
                                  <tr key={loc.locacao_id} className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                {reportData.length > 0 && (
                 ...
                 <div className="flex justify-end space-x-3 mt-6">
                    <button onClick={() => setStep(1)} className="px-4 py-2 bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-200 rounded-md hover:bg-gray-300 dark:hover:bg-gray-500">
                        Gerar Novo Relatório
                    </button>
                 </div>
              </div>
            )}
            {isGeneratingReport && step !== 3 && <p className="text-center text-gray-500 dark:text-gray-400 mt-4"><SpinnerIcon className="w-5 h-5 mr-2 inline" /> Gerando...</p>}
            {reportError && step !== 3 && <p className="text-red-500 dark:text-red-400 text-sm mt-3 text-center">{reportError}</p>}
          </div>
        </div>
      )}
